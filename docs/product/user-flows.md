...
| `/recent` | 최근 등록 곡 | 공개 | Recommendation 생성 시각 내림차순 목록 |
...
| `/join` | 계정 생성 | 공개 | 로그인 ID, 비밀번호, 복구용 이메일 입력 |
...
2. **최근 등록된 노래**: Recommendation 생성 시각 내림차순으로 제공한다. 오늘 등록된 곡에 한정하지 않고 최신 등록 순서를 유지한다.
...
과거 `FINAL` 차트에서는 당시 Vote 수와 순위 목록만 조회한다. Vote와 신규 Recommendation 행동은 노출하지 않으며 과거 snapshot을 변경하거나 오늘의 Vote로 전환하지 않는다. Preview와 외부 듣기는 순위 데이터에 영향을 주지 않는 탐색 행동으로 유지할 수 있다.
...
목적: 공개 닉네임과 분리된 로그인 ID로 기존 계정에 로그인한다.
...
- ID 찾기와 비밀번호 재설정은 이메일 인증 기반으로 추후 제공하며 MVP 화면에는 노출하지 않는다.
...
- 이메일은 공개되지 않으며 향후 ID 찾기와 비밀번호 재설정에만 사용한다는 안내
- 이메일 인증과 실제 계정 복구 기능은 MVP 이후 제공된다는 안내

성공 상태:

- 생성된 공개 닉네임 표시
- 로그인 ID와 이메일 보관 필요성 안내
...
닉네임 변경, 추천 이력, Taste Profile, 이메일 인증과 계정 복구는 표시하지 않는다. 준비 중인 기능을 빈 메뉴로 노출하지 않는다.
...
한줄평 신고 기능은 동일한 상태·메시지 원칙을 따르지만 MVP에서는 feature flag가 꺼져 있어 신고 버튼과 진입점을 렌더링하지 않고 신고 API도 비활성화한다.
...
| 내 계정 | 공개 닉네임, 로그인 ID, 마스킹된 이메일, 오늘의 추천 사용량 |
...
- 이메일은 공개 프로필 projection과 로그에서 제외하고 계정 소유자용 응답에서도 마스킹 여부를 명확히 해야 한다.
- 신고 도메인/API는 구현하되 feature flag가 꺼진 동안 사용자 UI를 숨기고 API 요청도 비활성 상태로 처리해야 한다.
...
6. 신고 중복 방지와 feature flag 적용 경계
